...
public class QualityStats {
    ...
    public static class RecallPoint {
        ...
        /**
         * Returns the rank: where on the list of returned docs this relevant
         * doc appeared.
         */
        public int getRank() {
            return rank;
    ...
    /**
     * Construct a QualityStats object with anticipated maximal number of
     * relevant hits.
     *
    ...
    public QualityStats(double maxGoodPoints, long searchTime) {
        ...
        rAt = new double[MAX_POINTS + 1]; // rAt[0] unused. 
        f1At = new double[MAX_POINTS + 1]; // f1At[0] unused.
    }
...
//        if (n < f1At.length) {
//            if (p != 0 && recall != 0) {
//                f1At[n] = 2 * p * recall / (p + recall);
//            } else {
//                f1At[n] = 0;
//                System.err.println(n + "\t" + p + "\t" + recall);
//            }
//        }
    ...
    /**
     * Return the precision at rank n: |{relevant hits within first
     * <code>n</code> hits}| / <code>n</code>.
     *
     * @param n requested precision point, must be at least 1 and at most
     * {@link #MAX_POINTS}.
     */
    ...
    public double getRecallAt(int n) {
        if (n < 1 || n > MAX_POINTS) {
            throw new IllegalArgumentException("n=" + n + " - but it must be in [1," + MAX_POINTS + "] range!");
        }
        if (n > numPoints) {
            return (numPoints * rAt[(int) numPoints]) / n;
        }
        return rAt[n];
    }
    ...
    public double getF1(int n) {
        ...
        if (n > numPoints) {
            return f1At[(int) numPoints];
        }
        double f1 = 0;
        if (pAt[n] > 0 && rAt[n] > 0) {
            f1 = 2 * pAt[n] * rAt[n] / (pAt[n] + rAt[n]);
        }
    ...
    /**
     * Create a QualityStats object that is the average of the input
     * QualityStats objects.
     *
        ...
        for (int i = 0; i < stats.length; i++) {
            ...
            if (stats[i].maxGoodPoints > 0) {
                ...
                for (int j = 1; j < avg.pAt.length; j++) {
                    ...
                    avg.rAt[j] += stats[i].getRecallAt(j);
                    avg.f1At[j] += stats[i].getF1(j);
                }
            }
        }
    ...
    /**
     * Returns the maximal number of good points. This is the number of relevant
     * docs known by the judge for the measured query.
     */
    ...
    /**
     * Returns the Mean reciprocal rank over the queries or RR for a single
     * query.
     * <p>
     * Reciprocal rank is defined as <code>1/r</code> where <code>r</code> is
     * the rank of the first correct result, or <code>0</code> if there are no
     * correct results within the top 5 results.
     * <p>
