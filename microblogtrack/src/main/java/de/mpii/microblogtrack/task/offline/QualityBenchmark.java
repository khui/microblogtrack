 ...
 * {@link org.apache.lucene.benchmark.quality.Judge Judge}. The second
 * configuration requires a non null
...
public class QualityBenchmark {
     ...
     * @param docNameField name of field containing the document name. This
     * allows to extract the doc name for search results, and is important for
     * judging the results.
     */
    ...
    public QualityStats[] execute(Judge judge, Query[] qs, SubmissionReport submitRep,
            PrintWriter qualityLog) throws Exception {
    ...
    private QualityStats analyzeQueryResults(QualityQuery qq, Query q, TopDocs td, Judge judge, PrintWriter logger, long searchTime) throws IOException {
        ...
        for (int i = 0; i < sd.length; i++) {
            String docName = extractTweetId(sd[i].doc);
    ...
    private String extractTweetId(int doc) throws IOException {
        return searcher.doc(doc).get(Configuration.TWEET_ID);
    }
