...
import de.mpii.microblogtrack.component.TweetSimilarity;
 ...
 * @author khui
 */
public class MaxReponSimilarity {

    static Logger logger = Logger.getLogger(MaxReponSimilarity.class.getName());

    private final List<QueryTweetPair> datapoints;
    ...
    public MaxReponSimilarity(List<QueryTweetPair> datapoint) {
        this.datapoints = datapoint;
        this.n = datapoints.size();
        this.datapointWeights = new double[datapoint.size()];
        updateNodeWeight();
        similarityMeasure = new TweetStringSimilarity();
    ...
    private void updateNodeWeight() {
        for (int i = 0; i < datapointWeights.length; i++) {
            datapointWeights[i] = datapoints.get(i).getRelScore();
        }
    }
    ...
    private void initSimimatrix() {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
                double similarity = similarityMeasure.similarity(datapoints.get(i), datapoints.get(j));
